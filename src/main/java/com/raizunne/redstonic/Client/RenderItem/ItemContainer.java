...
public class ItemContainer implements IItemRenderer {
    ...
    public void renderEntity(ItemStack stack) {
        ...
        // float rotationAngle = (float) (720.0 * (System.currentTimeMillis() & 0x3FFFL) / 0x3FFFL); //Credits to
        // BloodMagic for code example!
    ...
    public void renderInventory(ItemStack stack) {
        ...
        if (stack.stackTagCompound != null && stack.stackTagCompound.getInteger("block") != -1
            && fontRenderer != null
            && textureManager != null) {
            int number = stack.stackTagCompound.getInteger("number");
            ItemStack blockerino = new ItemStack(
                Block.getBlockById(stack.stackTagCompound.getInteger("block")),
                1,
                stack.stackTagCompound.getInteger("blockMeta"));
            GL11.glPushMatrix();
