    ...
    public static void init(RenderGameOverlayEvent event) {
        ...
        if (player.worldObj.isRemote) {
            ...
            if (player.getDistanceSq(mop.blockX + 0.5, mop.blockY + 0.5, mop.blockZ + 0.5) <= 25
                && Util.getCurrentItem(RedstonicItems.ManualBook)) {
                if (block instanceof DrillModifier) {
                    ...
                    minecraft.fontRenderer.drawStringWithShadow(
                        EnumChatFormatting.RED + "Manual Entry:" + EnumChatFormatting.RESET,
                        posx + 33,
                        posy - 8,
                        0x404040);
                    minecraft.fontRenderer.drawStringWithShadow(
                        EnumChatFormatting.GREEN + "Drill Modifier" + EnumChatFormatting.RESET,
                        posx + 33,
                        posy + 2,
                        0x404040);
            ...
            if (Util.getCurrentItem(RedstonicItems.RedDrill)) {
                if (player.isSneaking()) {
                    ...
                    if ((nbt.getInteger("aug1") == 4 || nbt.getInteger("aug2") == 4 || nbt.getInteger("aug3") == 4)
                        && nbt.getInteger("placeBlock") != 0) {
                        RenderItem.getInstance()
                            .renderItemIntoGUI(
                                minecraft.fontRenderer,
                                minecraft.renderEngine,
                                new ItemStack(
                                    Block.getBlockById(currentItem.stackTagCompound.getInteger("placeBlock")),
                                    0,
                                    currentItem.stackTagCompound.getInteger("placeBlockMeta")),
                                (event.resolution.getScaledWidth() / 2) + 93,
                                event.resolution.getScaledHeight() - 20);
                    }
                }
            }
        }
