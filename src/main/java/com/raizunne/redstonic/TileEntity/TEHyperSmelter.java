...
public class TEHyperSmelter extends TileEntity implements IInventory, ISidedInventory, IEnergyHandler {
    ...
    public void updateEntity() {
        if (getStackInSlot(0) != null || getStackInSlot(1) != null && getEnergyStored(null) >= 2000) {
            if (timer != maxTimer) {
                ...
                burn();
                timer = 0;
                // this.worldObj.playSound((double)this.xCoord, (double)this.yCoord, (double)this.zCoord,
                // "portal.trigger", 1F, 1F, true);
            }
        }
    }

    public void burn() {
        if (getStackInSlot(0) != null && FurnaceRecipes.smelting()
            .getSmeltingResult(getStackInSlot(0)) != null) {
            ItemStack burned = FurnaceRecipes.smelting()
        ...
        } else if (getStackInSlot(1) != null && FurnaceRecipes.smelting()
            .getSmeltingResult(getStackInSlot(1)) != null) {
                ItemStack burned = FurnaceRecipes.smelting()
                    .getSmeltingResult(getStackInSlot(1));
    ...
    public boolean checkFits(int count, ItemStack stack) {
        return getStackInSlot(2) == null || getStackInSlot(3) == null
            || (getStackInSlot(2).isItemEqual(stack) && getStackInSlot(2).stackSize + count <= 64)
            || (getStackInSlot(3).isItemEqual(stack) && getStackInSlot(3).stackSize + count <= 64);
    }
