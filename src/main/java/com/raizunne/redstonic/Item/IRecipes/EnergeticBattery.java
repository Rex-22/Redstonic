...
public class EnergeticBattery implements IRecipe {
    ...
    public boolean matches(InventoryCrafting inv, World world) {
        ...
        if (slot(0) == null && slot(1) == slot(6)
            && slot(6) == slot(8)
            && slot(8) == new ItemStack(Blocks.redstone_block)
            && slot(3) == OreDictionary.getOres("ingotEnergeticAlloy")
                .get(0)
            && slot(5) == slot(3)
            && slot(4) == OreDictionary.getOres("blockPhasedIron")
                .get(0)
            && slot(7).getItem() instanceof ItemBattery) {
