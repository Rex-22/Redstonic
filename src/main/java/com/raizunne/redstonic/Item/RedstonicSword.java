...
public class RedstonicSword extends Item implements IEnergyContainerItem {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean p_77624_4_) {
        if (stack.stackTagCompound != null) {
            ...
            if (stack.stackTagCompound.getInteger("MaxEnergy") == -1
                || stack.stackTagCompound.getInteger("MaxEnergy") == -2
                || stack.stackTagCompound.getInteger("MaxEnergy") == -5) {
                Energy = EnumChatFormatting.OBFUSCATED + "CREATIVE"
            ...
            list.add(
                EnumChatFormatting.GREEN + "Energy: " + EnumChatFormatting.GRAY + Energy + "/" + MaxEnergy + " RF");
            list.add(
                EnumChatFormatting.GOLD + "Kills: "
                    + EnumChatFormatting.GRAY
                    + stack.stackTagCompound.getInteger("totalKills"));
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                ...
                list.add(
                    Lang.translate("sword.Blade") + EnumChatFormatting.RESET
                        + EnumChatFormatting.DARK_GRAY
                        + ": "
                        + blade);
                list.add(
                    Lang.translate("sword.Handle") + EnumChatFormatting.RESET
                        + EnumChatFormatting.DARK_GRAY
                        + ": "
                        + handle);
                list.add(
                    Lang.translate("drill.battery") + EnumChatFormatting.RESET
                        + EnumChatFormatting.DARK_GRAY
                        + ": "
                        + battery);
                list.add(
                    Lang.translate("drill.energyusage") + EnumChatFormatting.RESET
                        + EnumChatFormatting.DARK_GRAY
                        + ": "
                        + EnumChatFormatting.GREEN
                        + Lang.addComas(calcEnergy(stack)));
                list.add(
                    Lang.translate("sword.Damage") + EnumChatFormatting.RESET
                        + EnumChatFormatting.DARK_GRAY
                        + ": "
                        + damage
                        + " Hearts");
            } else if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) && !hasNOAugment(stack)) {
                if (nbt.getInteger("aug1") != 0) {
                    list.add(
                        EnumChatFormatting.YELLOW + Lang.translate("drill.augment")
                            + ": "
                            + EnumChatFormatting.DARK_GRAY
                            + SwordUtil.getAugName(nbt.getInteger("aug1")));
                }
                if (nbt.getInteger("aug2") != 0) {
                    list.add(
                        EnumChatFormatting.YELLOW + Lang.translate("drill.augment")
                            + ": "
                            + EnumChatFormatting.DARK_GRAY
                            + SwordUtil.getAugName(nbt.getInteger("aug2")));
                }
                if (nbt.getInteger("aug3") != 0) {
                    list.add(
                        EnumChatFormatting.YELLOW + Lang.translate("drill.augment")
                            + ": "
                            + EnumChatFormatting.DARK_GRAY
                            + SwordUtil.getAugName(nbt.getInteger("aug3")));
                }
                // }else if(Keyboard.isKeyDown(56)){
                // list.add("MOTHERCUcKER");
            } else {
                list.add(
                    "Hold " + EnumChatFormatting.RED
    ...
    public int calcEnergy(ItemStack stack) {
        ...
        switch (head) {
            ...
            case 2:
                return (int) (1500 * multiplier);
            case 3:
                return (int) (3000 * multiplier);
            case 4:
                return (int) (1500 * multiplier);
            case 5:
                return (int) (3000 * multiplier);
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase entityLivingTo, EntityLivingBase entityLivingFrom) {
        ...
        boolean canHit = (stack.stackTagCompound.getInteger("Energy") >= calcEnergy(stack)
            || (stack.stackTagCompound.getInteger("MaxEnergy") == -2
                || stack.stackTagCompound.getInteger("MaxEnergy") == -1
                || stack.stackTagCompound.getInteger("MaxEnergy") == -5));
