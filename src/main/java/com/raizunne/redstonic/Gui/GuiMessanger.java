...
public class GuiMessanger extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int x, int y) {
        ...
        fontRendererObj.drawString(
            "Redstonic Messenger",
            posX + ((xSizeofTexture / 2) - (fontRendererObj.getStringWidth("Redstonic Messenger")) / 2),
            posY + 10,
            0x990000,
            false);
    ...
    public void initGui() {
        ...
        if (page != MENU) {
            ButtonDirectional dir = new ButtonDirectional(
                -3,
                posX + 16,
                posY + 9,
                ButtonDirectional.Type.LEFT_STRAIGHT);
            buttonList.add(dir);
        ...
        if (page == MENU) {
            for (int i = 0; i < menusT.length; i++) {
                menus[i] = new ButtonMenu(
                    i,
                    posX + ((xSizeofTexture / 2) - (fontRendererObj.getStringWidth(menusT[i])) / 2),
                    (posY + 25) + i * 12,
                    menusT[i],
                    color1,
                    color2,
            ...
            List<ItemStack> contents = RedstonicMessanger.getCratesFromPlayer(
                player,
