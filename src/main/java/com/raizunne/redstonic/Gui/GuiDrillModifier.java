...
public class GuiDrillModifier extends GuiContainer {
    ...
    public static final ResourceLocation texture = new ResourceLocation(
        "redstonic",
        "textures/gui/DrillModifierGUI.png");
    ...
    public void initGui() {
        ...
        ButtonWidget manual = new ButtonWidget(
            3,
            guiLeft - 21,
            guiTop + 5,
            "Manual",
            "left",
            RedstonicItems.ManualBook,
            itemRender);
        ...
        if (te.getStackInSlot(1) != null && te.getStackInSlot(2) != null
            ...
            && (te.checkForAssembleSword() || te.checkForAssembleDrill())) {
            this.buttonText = "Assemble";
            if (te.getStackInSlot(2) != null && te.getStackInSlot(2)
                .getItem() instanceof DrillBody) {
                ItemStack placeDrill = DrillUtil.getPlaceholderDrill(
                    DrillUtil.getHeadNumber(te.getStackInSlot(1)),
                    DrillUtil.getBodyNumber(te.getStackInSlot(2)),
                    DrillUtil.getAugNumber(te.getStackInSlot(5)),
                    DrillUtil.getAugNumber(te.getStackInSlot(6)),
                    DrillUtil.getAugNumber(te.getStackInSlot(7)));
                assemble.changeItem(placeDrill);
                secondLine = "Dig Speed: " + RedstonicDrill.getBaseDigSpeed(placeDrill, Blocks.stone);
            } else if (te.getStackInSlot(2) != null && te.getStackInSlot(2)
                .getItem() instanceof SwordHandle) {
                    ItemStack placeSword = SwordUtil.getPlaceholderSword(
                        SwordUtil.getBladeNumber(te.getStackInSlot(1)),
                        SwordUtil.getHandleNumber(te.getStackInSlot(2)),
                        SwordUtil.getAugNumber(te.getStackInSlot(5)),
                        SwordUtil.getAugNumber(te.getStackInSlot(6)),
                        SwordUtil.getAugNumber(te.getStackInSlot(7)));
                    assemble.changeItem(placeSword);
                    secondLine = SwordUtil.getAbsoluteDamage(placeSword) / 2 + " Heart Dmg";
                }
            buttonList.add(assemble);
        } else if (te.getStackInSlot(1) == null && te.getStackInSlot(2) == null
            && te.getStackInSlot(3) == null
            && te.getStackInSlot(0) != null
            && (te.getStackInSlot(0)
                .getItem() instanceof RedstonicDrill
                || te.getStackInSlot(0)
                    .getItem() instanceof RedstonicSword)) {
                        this.buttonText = "Disasssemble";
                        if (te.getStackInSlot(0) != null && te.getStackInSlot(0)
                            .getItem() instanceof RedstonicDrill) {
                            dissassemble.changeIcon(
                                DrillUtil.getDrillBody(te.getStackInSlot(0).stackTagCompound.getInteger("body")));
                        } else if (te.getStackInSlot(0) != null && te.getStackInSlot(0)
                            .getItem() instanceof RedstonicSword) {
                                dissassemble.changeIcon(
                                    SwordUtil.getHandle(te.getStackInSlot(0).stackTagCompound.getInteger("handle")));
                            }
    ...
    protected void keyTyped(char c, int i) {
        ...
        if (!((i == Keyboard.KEY_P || i == Keyboard.KEY_0
            || i == Keyboard.KEY_X
            || i == Keyboard.KEY_F
            || i == Keyboard.KEY_E) && this.rename.isFocused())) super.keyTyped(c, i);
    }
